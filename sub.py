...
class Mlp:
    def __init__(self, input_size, hidden_layer_sizes, output_size, activation, learning_rate, batch_size , weight_pattern , regulizer = "" , L1 = 0.0 , L2 = 0.0):
    ...
    def forwardprop(self, X):
        ...
        for i in range(len(self.layers)):
            ...
            if layer.output_size == self.output_size:
                ...
                layer.output = layer.act_func((np.dot(layer.input, layer.weights) + layer.bias))[0] #the zero is to get the function not the derivative of the act func
    ...
    def backwardprop(self, Y):
        ...
        for L in range(len(self.layers) - 1, -1, -1):
          ...
          if L == H:
              ...
              layer.dw = np.dot(layer.input.T, layer.dy)/Y.shape[1] #.shape[1] gives you columns, i.e. num of examples
          else:
    ...
    def gradientdescent(self, X, Y, NORMS):
        ...
        #norms = np.array([np.inf] * len(self.layers))  # Initialize array to store norms for each layer
        norms = NORMS
        #print("iteration :", t)
        ...
        for i, layer in enumerate(self.layers):
          #print("sizes are: ", "\n layer weights: ", layer.weights.shape , "\n layer derivative weights: ", layer.dw.shape, "\n layer output: ", layer.output.shape)
          # l2_regularized_weight = (1 - learning_rate * (self.l2 / n)) * w
          # updated_weight = l2_regularized_weight - (learning_rate / len(mini_batch)) * nw

          #layer.weights = layer.weights / np.sum((layer.weights))
          #print(np.sum(layer.weights))
    ...
    def fit(self, X, Y, num_iterations):
        ...
        while np.any(norms > self.epsilon) and t < self.max_iters:  # Check if all norms are smaller than epsilon
          ...
          for mini_batch in mini_batches:
            #if self.diffnorm < self.epsilon: break
...
start = time.time()
mlp = Mlp(input_size=784, hidden_layer_sizes=[128,128], output_size=10, activation="relu", learning_rate=0.1, batch_size=128 , weight_pattern="Uniform" , L2 = 0.1 , regulizer = "L2")
